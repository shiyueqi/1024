...
public class UpCommand extends AbstractCommand {
    ...
    public void doExecuteCommand() {
        ...
        for (int i = 0; i < NumsTable.NUMS_TABLE_SIZE; i++) {
            int zeroCount = 0;
            for (int j = 0; j < NumsTable.NUMS_TABLE_SIZE; j++) {
                if (nums[j][i] == 0) {
                    zeroCount++;
                }
            }
            ...
            //均不为0
            if (zeroCount == 0) {
                if (nums[0][i] == nums[1][i] && nums[2][i] == nums[3][i]) {
                    nums[0][i] = nums[0][i] * 2;
                    nums[1][i] = nums[2][i] * 2;

                    nums[2][i] = 0;
                    nums[3][i] = 0;
                } else if (nums[0][i] == nums[1][i] && nums[2][i] != nums[3][i]) {
                    nums[0][i] = nums[0][i] * 2;
                    nums[1][i] = nums[2][i];
                    nums[2][i] = nums[3][i];

                    nums[3][i] = 0;
                } else if (nums[0][i] != nums[1][i] && nums[1][i] == nums[2][i]) {
                    nums[1][i] = nums[1][i] * 2;
                    nums[2][i] = nums[3][i];

                    nums[3][i] = 0;
                } else if (nums[0][i] != nums[1][i] && nums[1][i] != nums[2][i] && nums[2][i] == nums[3][i]) {
                    nums[2][i] = nums[2][i] * 2;
