        ...
        for (int i = 0; i < NumsTable.NUMS_TABLE_SIZE; i++) {
            ...
            if (zeroCount == 0) {
                if (nums[i][0] == nums[i][1] && nums[i][2] == nums[i][3]) {
                    ...
                    nums[i][2] = 0;
                    nums[i][3] = 0;
                } else if (nums[i][0] == nums[i][1] && nums[i][2] != nums[i][3]) {
                    nums[i][0] = nums[i][0] * 2;
                    nums[i][1] = nums[i][2];
                    nums[i][2] = nums[i][3];

                    nums[i][3] = 0;
                } else if (nums[i][0] != nums[i][1] && nums[i][1] == nums[i][2]) {
                    nums[i][1] = nums[i][1] * 2;
                    nums[i][2] = nums[i][3];

                    nums[i][3] = 0;
                } else if (nums[i][0] != nums[i][1] && nums[i][1] != nums[i][2] && nums[i][2] == nums[i][3]) {
                    nums[i][2] = nums[i][2] * 2;
